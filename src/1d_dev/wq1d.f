SUBROUTINE WQ1D(ISTL_,IS2TL_)  
  ...
  ! --------------------Settings of Time variables before the calculation--------------------
  ...
  ! ITNWQ is a variable used to decide whehter it is the initial condition
  ! TIMEDAY is a variable from the hydrodynamic module
  IF( ITNWQ == 0 )THEN
  ...
  ! *** READ INITIAL CONDITIONS
  IF( ITNWQ == 0 )THEN  
    IF( IWQICI == 1 ) CALL WQICI ! IWQICI is from the input file

    ! *** READ TIME/SPACE VARYING ALGAE PARAMETERS  
    ! IWQAGR is from the input file
    IF( IWQAGR == 1 ) CALL WQAGR(IWQTAGR)   ! HARDWIRE FOR TEMPORALLY CONSTANT

    ! *** READ TIME/SPACE VARYING SETTLING VELOCITIES  start
    ! IWQSTL is from the input file
  ...
  CALL wqpsl ! TODO: Convert to python or Julia code

  ! --------------------Update Loadings during wet days--------------------
  CALL wqwet ! TODO: Convert to python or Julia code

  ! --------------------Calculate Physical Transport--------------------
  CALL calwqc ! TODO: work on modules called inside


  ! --------------Calculate Kinetics Sources and Sinks--------------------
  ...
  IF( ITNWQ == 0 .AND. IWQSUN > 1 .AND. NASER > 0 )THEN
    ...
    ! *** FIND 1ST POINT
    ! TSATM is related to input ASER.INP
    M = 1
  ...
  IF( ITNWQ == 0 .OR. WQKCNT >= WQKINUPT )THEN  
    ...
    DO M1=1,NWQTD  
      TT20 = WTEMP-20.0  
      WQTT = WQKFCB * WQTFCB**TT20 * DTWQO2  ! DTWQO2 = DTWQ*0.5
