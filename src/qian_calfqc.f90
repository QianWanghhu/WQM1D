SUBROUTINE CALFQC(ISTL_,IS2TL_,MVAR,MO,CON,CON1,IT)
  ...
  ! *** GROUNDWATER, EVAP, RAINFALL
  IF( ISGWIE/=0 )THEN  
    DO L=2,LA  
      FQC(L,KSZ(L),IT) = FQC(L,KSZ(L),IT) - RIFTR(L)*CONQ(L,KSZ(L),IT)  
    ENDDO  
  ENDIF  
  ...
  IF( M == 2 )THEN 
    IF( ISTOPT(2) == 0 .OR. ISTOPT(2) == 3 )THEN  
      ...
      ENDDO  
    ENDIF  
    IF( ISTOPT(2) == 1 .OR. ISTOPT(2) == 2 .OR. ISTOPT(2) == 4 )THEN  
      DO L=2,LA  
